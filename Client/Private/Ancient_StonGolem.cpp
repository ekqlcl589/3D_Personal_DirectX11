...
{
	if (m_PrevAnim == S_RUN && m_AnimTimeAcc >= 41.0 && m_AnimTimeAcc <= 42.0)// || m_AnimTimeAcc >= 79.0)
	{
		CSoundMgr::GetInstance()->StopSound(SOUND_GOLEM_EFFECT);
		CSoundMgr::GetInstance()->SoundPlay(L"FST_MON_Golem_Walk_Stone_01_A.ogg", SOUND_GOLEM_EFFECT, 1.0f);

	}

	if (m_PrevAnim == S_RUN && m_AnimTimeAcc >= 79.0)// && m_AnimTimeAcc <= 42.0)// || m_AnimTimeAcc >= 79.0)
	{
		CSoundMgr::GetInstance()->StopSound(SOUND_GOLEM_EFFECT);
		CSoundMgr::GetInstance()->SoundPlay(L"FST_MON_Golem_Walk_Stone_01_A.ogg", SOUND_GOLEM_EFFECT, 1.0f);
